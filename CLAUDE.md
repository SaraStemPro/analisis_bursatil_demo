...
**Incidente 2026-05-12 — la migración rota que mataba carteras**: `_migrate_close_order_status` (eliminada) corría en cada arranque del backend. Sumaba TODAS las órdenes `type='close'` históricas por `(portfolio_id, ticker, side)` y consumía esa cantidad contra cualquier `buy/sell` con `status='open'` del mismo `(pid, ticker, side)`, ignorando fecha y `portfolio_group`. Resultado: cualquier deploy podía machacar posiciones nuevas (típicamente dentro de carteras nuevas) si la alumna había cerrado en el pasado posiciones del mismo ticker. Fix: eliminada la migración + función one-shot `_restore_migration_victims` que reabre órdenes con `pnl IS NULL AND closed_at == created_at` (firma de la migración rota; los cierres legítimos ponen `pnl` y `closed_at > created_at`). NUNCA reintroducir esa migración: el sistema actual mantiene `status` correctamente en `_close_order_internal`.
