...
- **`get_screener` NO llama a `yf.Ticker(t).info` en vivo** (eso saturaba Yahoo y causaba el bug "11 productos de 258"). Solo lee de `_info_cache`. Para que la cache no esté vacía tras cada reinicio existe el thread `_info_prewarmer_loop` (arrancado en `main.py` vía `start_info_prewarmer()`) que rellena `.info` para todos los universos a 1 ticker cada 0.5 s. **Si quitas el prewarmer**, el screener vuelve a mostrar todos los tickers en rama "fallback" (precio + cambio OK pero sin sector / market cap / PE / dividend / beta / ROE / nombre real). **Si reintroduces `.info` síncrono en `get_screener`**, vuelves al rate-limit de Yahoo. Si tocas alguno de los dos, recuerda que el otro tiene que seguir cubriendo el flanco.
