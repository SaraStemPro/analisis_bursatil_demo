...
class ArrowRenderer implements IPrimitivePaneRenderer {
  ...
  constructor(x: number, y: number, direction: 'up' | 'down', color: string) {
    this.x = x; this.y = y; this.direction = direction; this.color = color
  }
  draw(target: CanvasRenderingTarget2D): void {
    drawArrow(target, this.x, this.y, this.direction, 14, this.color)
  }
}

class ArrowPaneView implements IPrimitivePaneView {
  _renderer: ArrowRenderer | null = null
...
export class ArrowPrimitive implements ISeriesPrimitive<Time> {
  ...
  constructor(drawing: ArrowDrawing) { this.drawing = drawing }
  ...
  _updateView(): void {
    ...
    this._paneView._renderer = new ArrowRenderer(p.x, p.y, this.drawing.direction, this.drawing.color)
  }
